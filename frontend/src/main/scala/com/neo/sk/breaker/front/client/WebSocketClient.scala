...
case class WebSocketClient(
                       ...
                       messageHandler:BreakerEvent.WsMsgServer => Unit,
  ...
  def sendMsg(msg:BreakerEvent.WsMsgFront) = {
      ...
      websocketStream.onmessage = { event: MessageEvent =>
        ...
        event.data match {
          ...
          case jsonStringMsg:String =>
            ...
            val data = decode[BreakerEvent.WsMsgServer](jsonStringMsg).right.get
            messageHandler(data)
  ...
  private def wsByteDecode(a:ArrayBuffer,s:Double):BreakerEvent.WsMsgServer={
    val middleDataInJs = new MiddleBufferInJs(a)
    bytesDecode[BreakerEvent.WsMsgServer](middleDataInJs) match {
      case Right(r) =>
        try {
          setDateSize(s"${r.getClass.toString.split("BreakerEvent").last.drop(1)}",s)
        }catch {case exception: Exception=> println(exception.getCause)}
        r
      case Left(e) =>
        println(e.message)
        BreakerEvent.DecodeError
    }
  }
